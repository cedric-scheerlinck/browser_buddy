...
async function loadClaudeApiKey() {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to load API key: ${response.status} ${response.statusText}`
      );
    }
    ...
    if (!trimmedKey) {
      throw new Error(
        "API key is empty. Please add your Claude API key to key.txt"
      );
    }
...
(async function init() {
  await loadApiKey();
  console.log(
    "Initial API key loaded, first 4 chars:",
...
async function callClaudeAPI(prompt, history, useAllTabs = false) {
  ...
  console.log(
    "API key available (first 4 chars):",
    apiKey.substring(0, 4) + "..."
  );
...
`
  ...
  console.log(
    `Using ${useAllTabs ? "All Tabs" : "Current Page"} system content`
  );
